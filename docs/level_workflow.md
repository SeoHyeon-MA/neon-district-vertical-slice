...
| `BLK_Boundary_N/S/E/W` | ±7550cm 경계 파사드 벽 (높이 15m). Ring 0.5의 위치 기준선 |
...
### `set_actor_transform`은 생략한 필드를 초기화한다

스키마 설명은 "설정하지 않은 필드는 변경하지 않는다"로 되어 있지만, 실제로는 **생략한 필드가
기본값으로 리셋된다.** 스케일만 바꾸려고 `xform`에 `scale`만 넘기면 위치가 `(0, 0, 0)`으로 날아간다.

실제로 벽 높이를 조정하면서 이 방식으로 10개 액터를 옮겨버렸고, 경계 파사드와 셔터, 광장 벽이
모두 맵 중앙에 겹쳐 쌓였다. 스크린샷에서는 다른 벽에 가려 바로 드러나지 않았다.

**기존 액터를 수정할 때는 `location` / `rotation` / `scale`을 항상 함께 넘긴다.**
바꾸지 않을 값은 `get_actor_transform`으로 읽어서 그대로 다시 넣는다.
수정 후에는 `get_actor_bounds`로 위치까지 검증한다. 높이만 확인하고 넘어가면 놓친다.
