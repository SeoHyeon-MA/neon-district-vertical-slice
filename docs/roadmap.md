...
| **9/28 ~ 10/4** | 전투 완성 + **환경 아트 착수** | 5 나머지, 7, 2 | 적 5명·순찰 경로 배치 / 감지 파라미터 조정(시야 거리·각도·Max Age·교전 거리) / 사망·재시작 화면, 획득 알림, 미션 완료 창 / **Ring 0 히어로 1곳(E 광장) 실제 에셋 교체 + 네온 라이팅 시험** | 8~12분 흐름 안에서 실제 전투. HUD만 보고 상황 파악 가능. 아트 파이프라인(키트→배치→라이팅)이 한 지점에서 검증됨 |
| **10/5 ~ 10/11** | 환경 아트 본작업 + 컷씬 | 7 나머지, 8 | 히어로 지점 나머지 2곳(A 진입부, F 2F 창) / 모듈러 키트로 그 외 구역 / Ring 1·2 원경 보강 / 시작 시퀀스 / **옥상 엔딩 시퀀스**(`docs/story.md`) / UI 톤 통일 | 처음부터 끝까지 연출이 연결된 빌드. 스크린샷 3장이 나오는 수준 |
...
적을 전투 구역에 고정 배치하고 감지 거리·각도·교전 거리 등 파라미터를 실제 플레이에 맞춰 조정한다. 사망·재시작 화면, 획득 알림, 미션 완료 창을 더해 HUD 세트를 마무리한다. 진행이 계획보다 앞서 있어 환경 아트를 이 주에 착수하여, 히어로 지점 한 곳을 실제 에셋과 네온 라이팅으로 교체하며 아트 파이프라인을 먼저 검증한다.
