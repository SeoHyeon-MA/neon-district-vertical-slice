...
- [ ] 대화 종료 시 미션 `AcceptMission()` 호출 (`NotAccepted → Accepted`)
- [ ] 미션 상태에 따른 대사 분기 (수락 전 / 진행 중 / 아이템 획득 후 복귀)
- [ ] 기획서의 "전화로 미션을 받는" 연출 — 대화 UI를 통화 형태로. 방향 표시·미니맵은 선택 사항이라 7번(UI)으로 미룬다
- [ ] 미션 완료 후 NPC를 종료 위치로 이동 (기획서 "npc 위치 옮기기")
...
**9/14 ~ 9/20 — 핵심 게임플레이 시스템 기반 구축**
Unreal FPS 템플릿의 데스매치 구조를 선형 미션 구조로 바꾼 전용 GameMode·Character·재시작 시스템 위에, NPC 대화·아이템·문·셔터가 공통으로 쓸 상호작용 시스템을 구현한다. 미션 액터의 상태 머신을 세부 단계까지 확장하고, 다른 시스템이 구독할 상태 변경 이벤트를 마련한다.

**9/21 ~ 9/27 — 미션 및 NPC 상호작용 구현**
Fixer NPC와의 통화 형태 대화와 미션 수락을 구현하고, 미션 상태에 따라 대사가 달라지도록 구성한다. 마지막 적이 떨어뜨리는 창고 키, 창고 개방, 아이템 획득, 셔터 개방을 미션 상태와 연동하여 수락부터 복귀·랭크 판정까지 하나의 흐름으로 연결한다.
