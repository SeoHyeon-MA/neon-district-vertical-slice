...
| `01_nanite_overhead.md` | Nanite 구성 전환 비용, 세션 간 재현성 확인 |
...
같은 조건으로 한 시간 간격을 두고 두 번 측정한 결과, 두 카메라 모두 약 **3.5% 낮게** 나왔다
(12.40 → 11.93ms, 13.42 → 12.98ms). 하드웨어 클럭이나 열 상태 같은 세션 단위 요인으로 보인다.
반면 **두 카메라 사이의 간격은 1.02 → 1.05ms로 거의 그대로**였다.

즉 이 측정 방식은 **절대값은 3~4% 흔들리지만 같은 세션 안의 상대 비교는 0.03ms 수준으로 안정적**이다.
따라서 전후 비교는 반드시 **같은 세션에서 연속으로** 측정한 값끼리만 한다.
어제 잰 값과 오늘 잰 값을 빼면 세션 편차가 결과에 그대로 섞이며,
1ms 안팎의 차이를 다루는 경우 그것만으로 결론이 뒤집힌다.

근거 수치는 `01_nanite_overhead.md`의 재현성 확인 절에 있다.
